...
class SaleFollowup(models.Model):
    ...
    def default_get(self, fields):
        ...
        message += "<p>Pl. find the enclosed PO. %s dtd %s received from %s on %s " \
                   "Our corresponding Order Request Number is %s .<br/>" \
                   "Kindly acknowledge the receipt of this order.</p>" \
                   % (sales_ids.po_no, sales_ids.po_date.strftime("%d-%m-%Y") if sales_ids.po_date else " ",
                      ...
                      sales_ids.po_received_date.strftime("%d-%m-%Y") if sales_ids.po_received_date else " ",
                      sales_ids.name)
        ...
        message += "ERP Team.<br/>"
        message += "Micro Abrasive Technologies,<br/>"
        message += "Chennai.</p>"
